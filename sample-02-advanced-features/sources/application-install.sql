...
CREATE SCHEMA s02 DEFAULT CHARACTER SET utf8mb4 DEFAULT COLLATE utf8mb4_unicode_ci;
...
SELECT
    t.TABLE_SCHEMA
    , t.TABLE_NAME
    , CAST(NULL AS CHAR CHARACTER SET utf8) AS COLUMN_NAME
    , 'Actions' AS EVENT_NAME
    , t.TABLE_SCHEMA AS HANDLER_SCHEMA
    , 'See Online Help' AS HANDLER_NAME
    , 'HTTP' AS HANDLER_TYPE
    , CONCAT('https://www.savetodb.com/samples/sample', SUBSTRING(t.TABLE_SCHEMA, 2, 2), '-', t.TABLE_NAME, CASE WHEN USER() LIKE 'sample%' THEN CONCAT('_', USER()) ELSE '' END) AS HANDLER_CODE
    , CAST(NULL AS CHAR CHARACTER SET utf8) AS TARGET_WORKSHEET
    , 1 AS MENU_ORDER
...
SELECT
    ...
    , 'See Online Help' AS HANDLER_NAME
    , 'HTTP' AS HANDLER_TYPE
    , CONCAT('https://www.savetodb.com/samples/sample', SUBSTRING(t.ROUTINE_SCHEMA, 2, 2), '-', t.ROUTINE_NAME, CASE WHEN USER() LIKE 'sample%' THEN CONCAT('_', USER()) ELSE '' END) AS HANDLER_CODE
    , CAST(NULL AS CHAR CHARACTER SET utf8) AS TARGET_WORKSHEET
    , 1 AS MENU_ORDER
    , 0 AS EDIT_PARAMETERS
FROM
...
WHERE
    ...
    AND (checked IS NULL OR p.checked = checked);
...
ELSEIF column_name = 'checked' THEN
    UPDATE s02.cashbook t SET checked = cell_number_value WHERE t.id = id;
...
SELECT
    ...
    , p.item_id
    , p.company_id
    , MAX(CASE WHEN p.company_id IS NOT NULL THEN CONCAT('  ', COALESCE(t1.TRANSLATED_NAME, p.name)) ELSE COALESCE(t1.TRANSLATED_NAME, p.name) END) AS `Name`
...
IF year IS NULL THEN SELECT YEAR(MAX(date)) INTO year FROM s02.cashbook; END IF;
...
IF month < 1 THEN SET month = NULL; END IF;

IF year IS NULL THEN SET year = YEAR(CURDATE()); END IF;

IF year IS NULL THEN SELECT YEAR(MAX(date)) INTO year FROM s02.cashbook; END IF;

SET start_date = DATE(CONCAT_WS('-', year, COALESCE(month, 1), 1));

SET end_date = DATE_ADD(DATE_ADD(DATE(CONCAT_WS('-', year, COALESCE(month, 12), 1)), INTERVAL 1 MONTH), INTERVAL -1 DAY);
...
INSERT INTO xls.handlers (TABLE_SCHEMA, TABLE_NAME, COLUMN_NAME, EVENT_NAME, HANDLER_SCHEMA, HANDLER_NAME, HANDLER_TYPE, HANDLER_CODE, TARGET_WORKSHEET, MENU_ORDER, EDIT_PARAMETERS) VALUES ('s02', 'usp_cash_by_months', NULL, 'ContextMenu', 's02', 'xl_details_cash_by_months', 'PROCEDURE', NULL, '_TaskPane', 11, NULL);
...
INSERT INTO xls.handlers (TABLE_SCHEMA, TABLE_NAME, COLUMN_NAME, EVENT_NAME, HANDLER_SCHEMA, HANDLER_NAME, HANDLER_TYPE, HANDLER_CODE, TARGET_WORKSHEET, MENU_ORDER, EDIT_PARAMETERS) VALUES ('s02', 'usp_cash_by_months', 'year', 'ParameterValues', 's02', 'xl_list_year', 'PROCEDURE', NULL, NULL, NULL, NULL);
